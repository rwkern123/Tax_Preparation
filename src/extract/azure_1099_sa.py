"""
Azure Document Intelligence opt-in extraction for Form 1099-SA.

NOTE: As of 2025, Azure Document Intelligence does not publish a dedicated
prebuilt-tax.us.1099SA model. This module uses the generic prebuilt-layout
model and maps extracted key-value pairs to Form1099SAData on a best-effort
...
def parse_1099_sa_azure(
    file_path: Path,
    endpoint: str,
    api_key: str,
) -> Optional["Form1099SAData"]:  # noqa: F821
    """
    ...
    try:
        ...
        logger.warning("azure_1099_sa: file not found: %s", file_path)
        return None
    except (HttpResponseError, ServiceRequestError) as exc:
        logger.warning("azure_1099_sa: Azure request failed: %s", exc)
        return None
    except Exception as exc:  # noqa: BLE001
        logger.warning("azure_1099_sa: unexpected error: %s", exc)
        return None
    ...
    earnings = _kv_float("earnings on excess")

    critical = [gross, earnings]
    populated = sum(1 for v in critical if v is not None)
    azure_confidence = round(min(1.0, populated / max(len(critical), 1)), 2)

    # Layout confidence is capped lower than dedicated models.
    ...
    full_text = " ".join(
        line.content for page in (result.pages or []) for line in (page.lines or [])
    )
    account_type: Optional[str] = None
    if re.search(r"Medicare\s+Advantage\s+MSA", full_text, re.IGNORECASE):
        account_type = "Medicare Advantage MSA"
    elif re.search(r"Archer\s+MSA", full_text, re.IGNORECASE):
        account_type = "Archer MSA"
    elif re.search(r"\bHSA\b|health\s+savings\s+account", full_text, re.IGNORECASE):
        account_type = "HSA"

    data = Form1099SAData(
        payer_name=_kv_str("trustee") or _kv_str("payer"),
        recipient_name=_kv_str("account holder") or _kv_str("recipient"),
        box1_gross_distribution=gross,
        box2_earnings_on_excess=earnings,
        box5_account_type=account_type,
        confidence=azure_confidence,
        extraction_source="azure",
    )

    logger.debug(
        "azure_1099_sa: layout-based extraction confidence %.2f for %s",
